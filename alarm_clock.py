...
#      ↓ 起きていたら → [finish]                  │
#      ↓ 寝ている                                  │
#  [ブザーON ＋ Dismissポップアップ表示]           │
#      ↓                                          │
#  [Dismiss押下 or 30秒経過]                       │
#      ↓                                          │
#  [ブザーOFF → 1分待機]                          │
#      ↓                                          │
#  [beat: 10*2^(n-1)回叩く(ブザーON継続)]          │
#      ↓                                          │
#  [n++]                                          │
#      └──────────────────────────────────────────┘
# =====================================================================
...
def alarm_sequence(stop_ev, snooze_ev, on_finish):
    """
    stop_ev  : 起床確認時にセットして全停止
    snooze_ev: 毎周のDismissボタン押下/30秒経過で次へ進むためのイベント
    on_finish: シーケンス終了時のGUIコールバック
    """
    n = 0  # 周回カウンタ。beat回数は 10*2^(n-1) (n=0は0回)
    try:
        while not stop_ev.is_set():
            ...
            # ── ステップ2: ブザーON ＋ beat (同時) ＋ Dismissポップアップ表示 ─
            ...
            # ハンマー打撃を別スレッドで並行実行 (30秒待ち中も叩き続ける)
            beat_count = 10 * (2 ** (n - 1)) if n > 0 else 0

            # snooze_ev を先にクリアしてからスレッド起動（競合防止）
            snooze_ev.clear()
            beat_stop = threading.Event()  # beat専用の停止イベント

            beat_thread = None
            if beat_count > 0:
                print("⏰ ブザー＋ハンマー {}回 (n={})".format(beat_count, n))
                beat_thread = threading.Thread(
                    target=hammer_strike, args=(beat_count, beat_stop), daemon=True
                )
                beat_thread.start()
            else:
                print("⏰ 初回につき叩かない (n=0)")

            # ウォッチャー: stop_ev/snooze_ev が立ったら beat_stop もセット
            # 引数で渡してクロージャ問題を回避する
            def make_watcher(bs, se, sne):
                def _watch():
                    while not bs.is_set():
                        if se.is_set() or sne.is_set():
                            bs.set()
                            return
                        sleep(0.1)
                return _watch
            ...
            watcher_thread = threading.Thread(
                target=make_watcher(beat_stop, stop_ev, snooze_ev), daemon=True
            )
            watcher_thread.start()

            # 毎周ポップアップを表示する (GUI操作はメインスレッドに依頼)
            root.after(0, show_alarm_popup)
            ...
            # 待機を抜けたらポップアップを閉じてビート/ブザーを止める
            root.after(0, close_alarm_popup)
            beat_stop.set()   # ハンマー停止
...
alarm_win = None  # 現在表示中のポップアップ（毎周作り直す）


def show_alarm_popup():
    """ブザーONのたびに毎周呼ばれ、Dismissポップアップを表示する"""
    global alarm_win
    ...
    def on_dismiss():
        """Dismissボタン: 30秒待ちをスキップさせる（シーケンスは続行）"""
...
def close_alarm_popup():
    """ポップアップが存在すれば閉じる"""
    global alarm_win
    if alarm_win is not None:
        try:
...
def show_clock():
    ...
    def raise_hammer():
        servo.angle = 90
        sleep(0.5)
    threading.Thread(target=raise_hammer, daemon=True).start()
